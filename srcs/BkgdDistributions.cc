/***
 * @file    BkgdDistributions.cc
 * @brief   Create a TTree filling with the 4-momenta of the initial and final state particles of the neutrino-argon interaction
 * @date    October 14th, 2018
 * @author  Yun-Tse Tsai (yuntse@slac.stanford.edu), Dane Stocks (dstocks@stanford.edu)
 * @version v1.0
 *
 * Currently we assume there is only one interaction, which is a neutrino-Argon interaction, in each event.
 * Therefore there should be only one MCTruth object in each event.
 * We will have to reconsider the scope of the TTree entries (per event or per interaction, how to count particles in two interactions nearby, etc.) when we start to consider multiple interactions (multiple neutrino-argon interactions).
...
void InitTree( TTree* pTree, CounterMap_t& Multiplicity, KinematicMap_t& Px, KinematicMap_t& Py, KinematicMap_t& Pz, KinematicMap_t& P, KinematicMap_t& E, KinematicMap_t& Angle, simb::Origin_t& EventOrigin, int& Mode, int& InteractionType, int& CCNC, int& isIn10kton, double& SunPx, double& SunPy, double& SunPz ) {
    ...
    pTree->Branch( "nInNus", &Multiplicity[-2000010000], "nInNus/i" );
    pTree->Branch( "nOutNus", &Multiplicity[2000010000], "nOutNus/i" );
    ...
    pTree->Branch( "InNuPx", &Px[-2000010000] );
    pTree->Branch( "OutNuPx", &Px[2000010000] );
    ...
    pTree->Branch( "InNuPy", &Py[-2000010000] );
    pTree->Branch( "OutNuPy", &Py[2000010000] );
    ...
    pTree->Branch( "InNuPz", &Pz[-2000010000] );
    pTree->Branch( "OutNuPz", &Pz[2000010000] );
    ...
    pTree->Branch( "InNuP", &P[-2000010000] );
    pTree->Branch( "OutNuP", &P[2000010000] );
    ...
    pTree->Branch( "InNuE", &E[-2000010000] );
    pTree->Branch( "OutNuE", &E[2000010000] );
    ...
    pTree->Branch( "InNuAngle", &Angle[-2000010000] );
    pTree->Branch( "OutNuAngle", &Angle[2000010000] );
...
int main( int argc, char ** argv ) {
    ...
    std::string SunFilename( argv[2] );
    // std::cout << "Sun position file: " << SunFilename << std::endl;
    std::vector< Momentum3_t > SunPositions = LoadSunPositions( SunFilename );
    ...
    TFile *fOut = new TFile( "BkgdSmearedMCParticles.root", "RECREATE" );
    ...
    int Mode, InteractionType, CCNC, isIn10kton;
    double SunPx, SunPy, SunPz;
    int nIn10kton = 0;
    
    InitTree( fTree, Multiplicity, Px, Py, Pz, P, E, Angle, EventOrigin, Mode, InteractionType, CCNC, isIn10kton, SunPx, SunPy, SunPz );
    ...
    for ( gallery::Event ev( Filenames ); !ev.atEnd(); ev.next() ) {
        ...
        Momentum3_t SunPosition = SunPositions[iSunPosition];
        ...
        auto& nInNu = Multiplicity[-2000010000];
        auto& nVisible = Multiplicity[2000000400];
        auto& nVisibleNoN = Multiplicity[2000000401];

        // Initialize the incident Nu particle.
        // Currently allow only one Nu-Argon interaction in an event
        auto& InNuPx = Px[-2000010000]; auto& InNuPy = Py[-2000010000]; auto& InNuPz = Pz[-2000010000];
        auto& InNuP = P[-2000010000]; auto& InNuE = E[-2000010000]; auto& InNuAngle = Angle[-2000010000];
        ResizeKinematics( InNuPx, InNuPy, InNuPz, InNuP, InNuE, InNuAngle, 1 );

        auto& OutNuPx = Px[2000010000]; auto& OutNuPy = Py[2000010000]; auto& OutNuPz = Pz[2000010000];
        auto& OutNuP = P[2000010000]; auto& OutNuE = E[2000010000]; auto& OutNuAngle = Angle[2000010000];
        ResizeKinematics( OutNuPx, OutNuPy, OutNuPz, OutNuP, OutNuE, OutNuAngle, 1 );
        ...
        for ( size_t iMCTruth = 0; iMCTruth < MCTruthObjs.size(); ++iMCTruth ) {
            ...
            // The incident Nu particle is stored in the neutrino container in the MCTruth
            simb::MCNeutrino const& InNuObj = MCTruthObj.GetNeutrino();
            simb::MCParticle const& InNu    = InNuObj.Nu();

            // Currently deal with only one Nu-Argon interaction in an event
            if ( nInNu > 0 ) {
                std::cout << "Found more than 1 Nu-Argon interaction in the event!" << std::endl;
                continue;
            }

            // Interaction information
            Mode = InNuObj.Mode();
            InteractionType = InNuObj.InteractionType();
            CCNC = InNuObj.CCNC();

            // Position of the vertex
            const auto Position = geo::vect::convertTo< Momentum4_t >( InNu.Position() );
            if ( abs( Position.X() ) <= 745.744 && abs( Position.Y() ) <= 600.019 && Position.Z() >= -0.87625 && Position.Z() <= 5808.87 ) {
              isIn10kton = 1;
              ++nIn10kton;
            } else isIn10kton = 0;
            
            // 4-momentum of the incident Nu particle
            const auto InNuMom = geo::vect::convertTo< Momentum4_t >( InNu.Momentum() );
            Event += InNuMom;

            Momentum3_t InNuMom3( InNu.Px(), InNu.Py(), InNu.Pz() );
            InNuPx[0] = InNu.Px(); InNuPy[0] = InNu.Py(); InNuPz[0] = InNu.Pz();
            InNuP[0] = InNu.P(); InNuE[0] = InNu.E(); InNuAngle[0] = CalculateAngle( SunPosition, InNuMom3 );

            // Now look for the initial argon 4-momentum and assign outgoing (final state) Nu kinematic values
            for ( size_t iMCParticle = 0; iMCParticle < MCTruthObj.NParticles(); ++iMCParticle ) {
                ...
                if ( thisMCParticle.StatusCode() == 1 && ( abs(thisMCParticle.PdgCode() ) == 12 ||abs(thisMCParticle.PdgCode() ) == 14 )  ) {
                    const auto Momentum = geo::vect::convertTo< Momentum4_t >( thisMCParticle.Momentum() );
                    OutNuPx[0] = Momentum.Px(); OutNuPy[0] = Momentum.Py(); OutNuPz[0] = Momentum.Pz();
                    OutNuP[0] = Momentum.P(); OutNuE[0] = Momentum.E();
                } else if ( thisMCParticle.StatusCode() == 1 ) continue;
                // The incident Nu particle has been filled
                if ( thisMCParticle.StatusCode() == 0 ) {
                    if ( abs( thisMCParticle.PdgCode() ) == 12 || abs( thisMCParticle.PdgCode() ) == 14 ) continue;
            ...
            Momentum3_t OutNuMom3( OutNuPx[0], OutNuPy[0], OutNuPz[0] );
            OutNuAngle[0] = CalculateAngle( SunPosition, OutNuMom3 );
            ...
            for ( size_t iMCParticle = 0; iMCParticle < G4MCParticles.size(); ++iMCParticle ) {
                ...
                if ( pdgCode > 2000000000 && pdgCode < 2000000301 ) {
                    ...
                    if ( abs( pdgCode ) == 12 || abs( pdgCode ) == 14 ) continue;
                    Visible += Momentum;
                    if ( Momentum.P() > LeadingParticleMax ) {
                        LeadingParticle = Momentum;
                        LeadingParticleMax = Momentum.P();
